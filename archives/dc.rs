...
async fn main() {
    ...
    for (tag, text) in elements {
        if tag == "p" {
            println!("{}:{}", tag, text);
            objects.p.push(converter::str_decimals(&text));
        } else if tag == "h1" {
            println!("{}:{}", yellow!(tag), yellow!(text));
            objects.h1.push(converter::str_decimals(&text));
        } else if tag == "h2" {
            println!("{}:{}", cyan!(tag), cyan!(text));
            objects.h2.push(converter::str_decimals(&text));
        } else if tag == "h3" {
            println!("{}:{}", magenta!(tag), magenta!(text));
            objects.h3.push(converter::str_decimals(&text));
        } else if tag == "a" {
            println!("{}:{}", blue!(tag), blue!(text));
            objects.a.push(converter::str_decimals(&text));
        }
    }
